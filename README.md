...
plant — no intersecting tubes, no "place a twig on a branch" merges. It now includes a dedicated desert
plant pass for AAA-ready cacti and succulents: Saguaro, Golden Barrel, Organ Pipe, Prickly Pear,
Agave, Aloe and Ocotillo are generated procedurally with no image assets. Branches grow *out of* their
...
a single skin. 30 tree species, 14 grasses and 7 desert plant species ship as presets.
...
| Desert plants | `src/plant/desertMesher.ts`, `src/plant/desertParams.ts` | **Welded cactus / succulent mesher**: ribbed columns, collision-planned arms, flattened pads, rosette leaves and ocotillo whips. Procedural detail geometry adds areoles, individual spines, flowers, serrated margins and small ocotillo leaves; 7 presets, no image assets. |
...
The desert family is a separate generator, not a tree preset with a cactus texture. `DesertMesher`
constructs a ribbed body and plans every arm or pad against the parent ring grid before meshing it.
The attachment is an actual rectangular window bridged with collar loops, so Saguaro and Organ Pipe
arms, Prickly Pear daughter pads and Ocotillo whips stay a single closed genus-0 surface.

The surface detail pass is deterministic for the plant seed and produces areole discs, individual
spines, terminal flowers, agave/aloe leaf ribbons and margin teeth. Detail geometry is coloured in the
worker response; there are no downloaded or generated image assets in the generator. The desert presets
are run through the same topology gate as trees and grasses: zero boundary edges, zero non-manifold
edges, zero dropped organs and 100% quad body topology are required before a preset is accepted.
...
`scripts/probe.ts` (trees), `scripts/grassProbe.ts` (grasses) and `scripts/desertProbe.ts` (cacti
and succulents) run the full pipeline headlessly and print the topology audit per preset × seed:
`npx vite-node scripts/desertProbe.ts "" 1,7,42`.

`npm test` covers every preset (30 trees + 14 grasses + 7 desert plants) × 3 seeds: closed, manifold,
consistently wound, one component, genus 0, quad ratio, drop budget, wind attribute ranges,
determinism and the OBJ/GLB/GPU exporters. Desert tests additionally require zero dropped organs,
colour-complete detail geometry and a connected shared-pipeline export.

## Scope of this phase

The generator stays procedural and asset-free: solid-colour botanical materials, welded quad bodies,
plus coloured detail geometry for cactus areoles, individual spines, flowers and succulent leaf
ribbons. Bark and blade texture atlases and LOD chains remain future work; the quad flow and UV seams
are already laid out to make those additions straightforward. Grasses and the desert family are
separate non-tree meshers; shrubs and ferns can reuse the same organ/window machinery without
weakening the topology gate.
