...
describe('desert cactus and succulent surfaces', () => {
  for (const preset of DESERT_PRESETS) {
    for (const seed of [1, 7, 42]) {
      it(`${preset.name} seed ${seed} is a closed genus-0 surface`, () => {
        ...
        expect(report.genus).toBe(0);
        expect(report.quadRatio).toBe(1);
        expect(built.stats.dropped).toBe(0);
        expect(built.height).toBeGreaterThan(0.1);
      });
    }
  }

  it('desert presets use the shared generation, validation and exporters', () => {
    const p = cloneParams(PRESETS.find((x) => x.name === 'Agave Americana')!);
