...
export type PlantPalette = 'tree' | 'grass' | 'desert';
...
const CACTUS = new THREE.Color(0x5f8c67);
const GRASS_BODY = new THREE.Color(0x6e8650);
...
export class Viewer {
  ...
  setTree(buffers: GpuBuffers, leaves: LeafMesh | null, height: number, palette: PlantPalette = 'tree'): void {
    this.clearTree();
    if (palette === 'desert') {
      this.groundMat.color.setHex(0x3a3026);
      this.scene.background = new THREE.Color(0x120f0c);
    } else if (palette === 'grass') {
      this.groundMat.color.setHex(0x1e2921);
      this.scene.background = new THREE.Color(0x0c1110);
    } else {
      this.groundMat.color.setHex(0x1a1c20);
      this.scene.background = new THREE.Color(0x0c0d10);
    }
    ...
    const mat = this.makeBarkMaterial(palette);
    ...
    if (leaves && leaves.count > 0) {
      ...
      if (leaves.colors.length === leaves.positions.length && leaves.colors.length > 0) {
        lgeo.setAttribute('color', new THREE.BufferAttribute(new Float32Array(leaves.colors), 3));
      }
      lgeo.setIndex(new THREE.BufferAttribute(new Uint32Array(leaves.indices), 1));
      const lmat = this.makeLeafMaterial(leaves.colors.length === leaves.positions.length && leaves.colors.length > 0);
  ...
  private makeBarkMaterial(palette: PlantPalette = 'tree'): THREE.MeshStandardMaterial {
    const mat = new THREE.MeshStandardMaterial({
      color: palette === 'desert' ? CACTUS : palette === 'grass' ? GRASS_BODY : BARK,
      roughness: palette === 'desert' ? 0.78 : 0.92,
      metalness: 0.0,
  ...
  private makeLeafMaterial(vertexColors = false): THREE.MeshStandardMaterial {
    const mat = new THREE.MeshStandardMaterial({ color: LEAF, roughness: 0.75, metalness: 0, side: THREE.DoubleSide, vertexColors });
