...
import { DesertParams, DesertForm, DESERT_FORM_NAMES, desertHeight, desertHabit } from './plant/desertParams';
...
function syncTabs(): void {
  ...
  for (const [b] of tabMap) {
    const hide = (grass || desert) ? treeTabs.has(b) : grassTabs.has(b) || desertTabs.has(b);
    if (b === tabGrass) b.style.display = grass ? '' : 'none';
    else if (b === tabDesert) b.style.display = desert ? '' : 'none';
    else b.style.display = hide ? 'none' : '';
  }
  const active = tabMap.find(([b]) => b.classList.contains('on'))?.[0];
  if (!active || active.style.display === 'none') showTab(desert ? tabDesert : grass ? tabGrass : tabBotany);
  inspectorBadge.textContent = desert ? 'desert · welded' : grass ? 'grass · welded' : 'tree · welded';
}
showTab(tabBotany);
...
function baseReach(): number {
  ...
  if (isDesert(params) && params.desert) return Math.max(0.25, params.desert.radius * 3);
...
function syncView(): void {
  ...
  legend.innerHTML = '';
  if (view.mode === 'levels') {
    const items: [string, string][] = isDesert(params)
      ? [
          ['Core', '#5f8c67'],
          ['Arms / pads', '#8ab66d'],
          ['Ribs', '#c5a16c'],
...
worker.onmessage = (ev: MessageEvent<WorkerResponse>) => {
  ...
  if (msg.type === 'generated') {
    ...
    leaves.colors = Array.from(msg.leaves.colors);
    leaves.indices = Array.from(msg.leaves.indices);
    const firstTree = !hasTree;
    viewer.setTree(buffers, leaves, msg.summary.height, isDesert(params) ? 'desert' : isGrass(params) ? 'grass' : 'tree');
...
function groupIcon(name: string): string {
  ...
  if (g === 'Desert' || g.startsWith('Desert ·')) return 'yucca';
  return 'tree';
}
...
function updateReport(r: GenerateResponse): void {
  ...
  if (desert && r.desert) {
    cell('arms', fmtInt(r.desert.arms));
    cell('spines', fmtCompact(r.desert.spines));
    cell('details', fmtCompact(r.desert.details));
  } else if (grass && r.grass) {
  ...
  if (desert) drawSpark(spark, r.summary.stemsPerLevel.slice(0, 4), 0, ['core', 'arms', 'pads', 'detail']);
  else if (grass) drawSpark(spark, r.summary.stemsPerLevel.slice(0, 4), 0, ['crown', 'culm', 'leaf', 'head']);
  ...
  vtitleMeta.textContent = desert || grass
  ...
  const parts = desert || grass ? r.summary.stemsPerLevel.slice(0, 4) : [...r.summary.stemsPerLevel.slice(0, 4), r.summary.rootStems];
  ...
  if (desert) footRow.append(kvSpan('yucca', 'organs', fmtInt(r.summary.stems)), kvSpan('layers', 'junctions', fmtInt(r.stats.junctions)), el('span', 'sp'), kvSpan('check', ok ? 'clean' : 'issues', '', ok ? 'ok' : 'warn'));
  else if (grass) footRow.append(kvSpan('grass', 'organs', fmtInt(r.summary.stems)), kvSpan('layers', 'junctions', fmtInt(r.stats.junctions)), el('span', 'sp'), kvSpan('check', ok ? 'clean' : 'issues', '', ok ? 'ok' : 'warn'));
  ...
  if (desert && r.desert) {
    add('Organs welded', `${fmtInt(r.desert.organs)}  (${r.desert.junctions} windows)`);
    add('Arms · pads', `${fmtInt(r.desert.arms)} · ${fmtInt(r.desert.pads)}`);
    add('Spines', fmtInt(r.desert.spines));
    add('Rosette leaves', fmtInt(r.desert.rosetteLeaves));
    add('Flowers · details', `${fmtInt(r.desert.flowers)} · ${fmtInt(r.desert.details)}`);
    add('Organs dropped', fmtInt(r.desert.dropped), r.desert.dropped ? 'warn' : 'ok');
  } else if (grass && r.grass) {
...
const GROUP_ACCENT: Record<string, string> = {
  ...
  'Desert · cacti & succulents': '#d8a15d',
};
...
function speciesMeta(name: string): string {
  ...
  if (isDesert(p) && p.desert) {
    const d = p.desert;
    const h = desertHeight(d);
    return `${desertHabit(d)} · ${h < 1 ? `${Math.round(h * 100)} cm` : `${h.toFixed(1)} m`} · ${d.ribs ? `${d.ribs} ribs` : `${d.rosetteLeaves} leaves`}`;
  }
  const b = p.botany;
...
function buildDesertPage(): void {
  ...
  const D = (): DesertParams => params.desert!;
  ...
  const reset = button('Reset species', () => {
    const seed = params.seed;
    params = cloneParams(PRESETS.find((p) => p.name === params.name) ?? PRESETS[0]);
    params.seed = seed;
    rebuildPages();
    scheduleGenerate(true);
  }, { icon: 'reset', title: 'Restore the measured starting values of the current desert species' });
  sSeed.lastElementChild?.append(reset);
  refreshers.push(() => {
    seedInput.value = String(params.seed);
  });

  const sForm = section('Form & silhouette', page, { hint: desertHabit(D()) });
  select(
    sForm,
    'Plant form',
    (Object.keys(DESERT_FORM_NAMES) as DesertForm[]).map((k) => ({ value: k, label: DESERT_FORM_NAMES[k] })),
    () => D().form,
    (v) => (D().form = v),
    () => {
      onChange();
  ...
  slider(sForm, 'Height', () => D().height, (v) => (D().height = v), { min: 0.12, max: 8, step: 0.01, unit: 'm' }, onChange);
  slider(sForm, 'Height variation', () => D().heightV, (v) => (D().heightV = v), { min: 0, max: 0.35, step: 0.01, unit: '±' }, onChange);
  slider(sForm, 'Body radius', () => D().radius, (v) => (D().radius = v), { min: 0.03, max: 1.2, step: 0.005, unit: 'm' }, onChange);
  slider(sForm, 'Radius variation', () => D().radiusV, (v) => (D().radiusV = v), { min: 0, max: 0.5, step: 0.01, unit: '±' }, onChange);
  slider(sForm, 'Lean', () => D().lean, (v) => (D().lean = v), { min: 0, max: 12, step: 0.1, unit: '°' }, onChange);
  slider(sForm, 'Curve', () => D().curve, (v) => (D().curve = v), { min: 0, max: 18, step: 0.2, unit: '°' }, onChange);
  slider(sForm, 'Taper', () => D().taper, (v) => (D().taper = v), { min: 0, max: 0.7, step: 0.01, title: 'Amount the body narrows towards the crown' }, onChange);
  slider(sForm, 'Shoulder rounding', () => D().shoulder, (v) => (D().shoulder = v), { min: 0, max: 0.8, step: 0.01, title: 'Rounds the terminal shoulder without collapsing the cap' }, onChange);

  const sArms = section('Arms & pads', page, { hint: D().form === 'prickly-pear' ? `${D().padCount} pads` : `${D().arms} arms` });
  slider(sArms, 'Arms', () => D().arms, (v) => (D().arms = Math.round(v)), { min: 0, max: 24, step: 1, title: 'Primary arms or whip stems' }, onChange);
  slider(sArms, 'Attach height', () => D().armHeight, (v) => (D().armHeight = v), { min: 0.02, max: 0.9, step: 0.01, unit: '× H' }, onChange);
  slider(sArms, 'Height variation', () => D().armHeightV, (v) => (D().armHeightV = v), { min: 0, max: 0.35, step: 0.01, unit: '±' }, onChange);
  slider(sArms, 'Arm / pad length', () => D().armLength, (v) => (D().armLength = v), { min: 0.1, max: 4, step: 0.01, unit: 'm' }, onChange);
  slider(sArms, 'Arm radius', () => D().armRadius, (v) => (D().armRadius = v), { min: 0.015, max: 0.55, step: 0.005, unit: 'm' }, onChange);
  slider(sArms, 'Arm curve', () => D().armCurve, (v) => (D().armCurve = v), { min: 0, max: 30, step: 0.5, unit: '°' }, onChange);
  slider(sArms, 'Arm taper', () => D().armTaper, (v) => (D().armTaper = v), { min: 0, max: 0.8, step: 0.01 }, onChange);
  stepper(sArms, 'Daughter arms', () => D().secondaryArms, (v) => (D().secondaryArms = v), { min: 0, max: 2, integer: true, title: 'Second-generation arms / pads on each primary' }, onChange);
  slider(sArms, 'Pad width', () => D().padWidth, (v) => (D().padWidth = v), { min: 0.12, max: 1.4, step: 0.01, unit: 'm' }, onChange);
  slider(sArms, 'Pad height', () => D().padHeight, (v) => (D().padHeight = v), { min: 0.18, max: 1.6, step: 0.01, unit: 'm' }, onChange);
  slider(sArms, 'Pad thickness', () => D().padThickness, (v) => (D().padThickness = v), { min: 0.03, max: 0.3, step: 0.005, unit: 'm' }, onChange);
  slider(sArms, 'Pad tilt', () => D().padTilt, (v) => (D().padTilt = v), { min: -35, max: 70, step: 1, unit: '°' }, onChange);
  stepper(sArms, 'Pad count', () => D().padCount, (v) => (D().padCount = v), { min: 1, max: 14, integer: true }, onChange);

  const sLeaves = section('Rosette leaves', page, { hint: `${D().rosetteLeaves} leaves` });
  stepper(sLeaves, 'Leaves', () => D().rosetteLeaves, (v) => (D().rosetteLeaves = v), { min: 4, max: 120, integer: true }, onChange);
  slider(sLeaves, 'Length', () => D().leafLength, (v) => (D().leafLength = v), { min: 0.15, max: 2.5, step: 0.01, unit: 'm' }, onChange);
  slider(sLeaves, 'Width', () => D().leafWidth, (v) => (D().leafWidth = v), { min: 0.03, max: 0.45, step: 0.005, unit: 'm' }, onChange);
  slider(sLeaves, 'Lean', () => D().leafLean, (v) => (D().leafLean = v), { min: 8, max: 88, step: 1, unit: '°' }, onChange);
  slider(sLeaves, 'Droop', () => D().leafDroop, (v) => (D().leafDroop = v), { min: 0, max: 70, step: 1, unit: '°' }, onChange);
  slider(sLeaves, 'Edge teeth', () => D().leafSerration, (v) => (D().leafSerration = v), { min: 0, max: 1, step: 0.05, title: 'Small margin teeth on agave and aloe leaves' }, onChange);

  const sSurface = section('Ribs, areoles & flowers', page);
  stepper(sSurface, 'Ribs', () => D().ribs, (v) => (D().ribs = v), { min: 0, max: 40, integer: true, title: 'Longitudinal body ribs; 0 for pads and rosettes' }, onChange);
  slider(sSurface, 'Rib depth', () => D().ribDepth, (v) => (D().ribDepth = v), { min: 0, max: 0.22, step: 0.005 }, onChange);
  slider(sSurface, 'Rib sharpness', () => D().ribSharpness, (v) => (D().ribSharpness = v), { min: 0.5, max: 4, step: 0.1 }, onChange);
  stepper(sSurface, 'Areole rows', () => D().areoleRows, (v) => (D().areoleRows = v), { min: 0, max: 36, integer: true }, onChange);
  stepper(sSurface, 'Areole columns', () => D().areoleColumns, (v) => (D().areoleColumns = v), { min: 0, max: 40, integer: true }, onChange);
  stepper(sSurface, 'Spines / areole', () => D().spinesPerAreole, (v) => (D().spinesPerAreole = v), { min: 0, max: 12, integer: true }, onChange);
  slider(sSurface, 'Spine length', () => D().spineLength, (v) => (D().spineLength = v), { min: 0.002, max: 0.16, step: 0.001, unit: 'm' }, onChange);
  check(sSurface, 'Terminal flowers', () => D().flower, (v) => (D().flower = v), onChange);
  slider(sSurface, 'Flower scale', () => D().flowerScale, (v) => (D().flowerScale = v), { min: 0.03, max: 0.5, step: 0.005, unit: 'm' }, onChange);

  const sRes = section('Resolution', page, { collapsed: true });
  stepper(sRes, 'Body radial segments', () => D().radialSegments, (v) => (D().radialSegments = v), { min: 12, max: 64, step: 2, integer: true }, onChange);
  stepper(sRes, 'Body rings', () => D().bodyRings, (v) => (D().bodyRings = v), { min: 8, max: 64, integer: true }, onChange);
  stepper(sRes, 'Arm rings', () => D().armRings, (v) => (D().armRings = v), { min: 6, max: 32, integer: true }, onChange);
  stepper(sRes, 'Collar rings', () => D().collarRings, (v) => (D().collarRings = v), { min: 0, max: 4, integer: true }, onChange);

  page.append(
    Object.assign(el('p', 'note'), {
      textContent:
        'Cacti and succulents are generated as real, collision-free surfaces: ribs are sampled in the body cross-section, every arm or pad exits through a welded collar, and the detail pass adds areoles, spines, flowers and serrated rosette leaves. The topology report is a hard quality gate; a plant with an open edge is never presented as clean.',
    }),
  );
}
...
(window as unknown as { frontier: unknown }).frontier = {
  ...
  setParams(partial: {
    ...
    if (partial.desert && params.desert) Object.assign(params.desert, partial.desert);
