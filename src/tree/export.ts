 ...
 * consumed directly by a game engine material. Plant-specific detail layers
 * may replace that colour stream with authored procedural RGB values.
 */
...
export function toGLB(mesh: QuadMesh, leaves: LeafMesh | null, name = 'tree'): ArrayBuffer {
  ...
  if (leaves && leaves.count > 0) {
    ...
    const hasLeafColors = leaves.colors.length === leaves.positions.length && leaves.colors.length > 0;
    const lcol = hasLeafColors ? new Float32Array(leaves.colors) : new Float32Array(leaves.wind);
    ...
    primitives.push({
      attributes: {
        ...
        COLOR_0: addAccessor(lcol, hasLeafColors ? 'VEC3' : 'VEC4', 34962),
      },
