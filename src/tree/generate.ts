...
export interface GenerateResult {
  /** Tree skeleton (null for grasses and desert plants). */
  ...
  grass?: GrassStats;
  /** Cactus / succulent mesher statistics (desert plants only). */
  desert?: DesertStats;
}
...
export function completeParams(params: TreeParams): TreeParams {
  ...
  if (isDesert(p) && !p.desert) p.desert = { ...DEFAULT_DESERT };
...
/** Cacti and succulents use a welded body plus a coloured detail layer for spines, flowers and leaves. */
function generateDesert(params: TreeParams, options: { validate?: boolean; obstacleMeshes?: boolean }): GenerateResult {
  const d = params.desert ?? { ...DEFAULT_DESERT };
  params.desert = d;
  const t0 = now();
